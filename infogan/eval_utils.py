...
def prepare_data_for_gan(x, nz, nl, nc, device):
    r"""
    Helper function to prepare inputs for model.
    """
    batch_size = x.size(0)
    sampled_labels = np.random.randint(nl, size=batch_size)
    return (
        x.to(device),
        torch.randn((batch_size, nz)).to(device),
        to_categorical(sampled_labels, num_columns=nl).to(device),
        torch.rand(batch_size, nc, device=device)*2 - 1,
    )
...
def compute_loss_g(net_g, net_d, z, label, code, loss_func_g):
    r"""
    General implementation to compute generator loss.
    """

    fakes = net_g(z, label, code)
    fake_preds,_ ,_ = net_d(fakes)
...
def compute_loss_d(net_g, net_d, reals, z, label, code, loss_func_d):
    r"""
    General implementation to compute discriminator loss.
    """
    real_preds,_,_ = net_d(reals)
    real_preds = real_preds.view(-1)
    fakes = net_g(z, label, code).detach()
    fake_preds,_,_ = net_d(fakes)
    fake_preds = fake_preds.view(-1)
...
def evaluate(net_g, net_d, dataloader, nz, nl, nc, device, samples_z=None):
    r"""
    ...
    with torch.no_grad():
        ...
        for data, _ in tqdm(dataloader, desc="Evaluating Model"):
            ...
            reals, z, label, code = prepare_data_for_gan(data, nz, nl, nc, device)
